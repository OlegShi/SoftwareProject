...
typedef enum sp_config_msg_t {
	...
	SP_CONFIG_SUCCESS,
	SP_CONFIG_INVALID_CONFIGURATION_FILE,
	SP_CONFIG_INVALID_BOOLEAN,
	SP_CONFIG_INVALID_ENUM_KDTREE
} SP_CONFIG_MSG;

typedef struct sp_config_t* SPConfig;

typedef enum KDTreeSplitMethod {
 ...
 * - SP_CONFIG_INVALID_CONFIGURATION_FILE - if a line in the config file is not an empty line or
 *   comment line or system variable line
 * - SP_CONFIG_INVALID_BOOLEAN - if a line in the config file contains invalid boolean
 * - SP_CONFIG_INVALID_ENUM_KDTREE - if a line in the config file contains invalid enum KDTreeSplitMethod
 *
 */
...
/**
 * Given an index 'index' and the string extension 'extensionFeats the function stores in
 * imageFeatsPath the full path of the ith image.
 *
 * For example:
 * Given that the value of:
 *  spImagesDirectory = "./images/"
 ...
 *  index = 10
 *
 * The functions stores "./images/img10.feats" to the address given by imageFeatsPath.
 * Thus the address given by imageFeatsPath must contain enough space to
 * store the resulting string.
 *
 * @param imageFeatsPath - an address to store the result in, it must contain enough space.
 * @param config - the configuration structure
 * @param index - the index of the image.
 * @param extensionFeats - the extension of the file
 *
 * @return
 * - SP_CONFIG_INVALID_ARGUMENT - if imagePath == NULL or config == NULL or extensionFeats == NULL
 * - SP_CONFIG_INDEX_OUT_OF_RANGE - if index >= spNumOfImages
 * - SP_CONFIG_SUCCESS - in case of success
 */
